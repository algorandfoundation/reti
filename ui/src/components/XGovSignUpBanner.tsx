...
export function XGovSignUpBanner({ validator }: XGovSignUpBannerProps) {
  ...
  const requestedPools = Object.keys(requests.data ?? {}).map(key => requests.data![Number(key)].xgovAddr)
  const unenrolledPools = pools.filter(p => !xgovs.data?.[p] && !requestedPools.includes(p))

  let enrollMessage = <>Enroll {numEnrollable} Pool{numEnrollable > 1 ? "s" : ""}</>
  ...
  return (
    <div className="mt-6 md:rounded-xl mx-auto max-w-3xl w-full bg-algo-blue dark:bg-algo-teal">
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start p-4">
        <div>
          <h2 className="text-lg mb-1 text-center md:text-left font-semibold text-white dark:text-algo-black">xGov Pool Manager Sign Up</h2>
